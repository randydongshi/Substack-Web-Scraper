...
def substack_post_scraper(url):
    ...
    writer1.writerow(['Post Link', 'Post Content', 'Word Count'])

    file2 = open(input('Name the file for post comment data'), 'w')
    writer2 = csv.writer(file2)
    writer2.writerow(['Post Link', 'Commenter', 'Comment', 'Commenter Writes'])
...
for link in links:
    browser.get(link)
    try:
        post_content_raw = browser.find_element_by_css_selector('div.available-content > div').text
        wordcount = len(post_content.replace(',', '').split())
    except:
        try:
            post_content_raw = browser.find_element_by_css_selector('td > div').text
            wordcount = len(post_content.replace(',', '').split())
        except:
            post_content_raw = browser.find_element_by_class_name('body markup').text
            wordcount = len(post_content.replace(',', '').split())
    try:
        writer1.writerow([link, post_content, wordcount])
    except:
        writer1.writerow([link.encode('utf-8'), post_content.encode('utf-8'), wordcount])
    try:
    ...
    for comment in comments:
        try:
            ...
            commenter = "Error"
        try:
            actual_comment = comment.find_element_by_class_name('comment-body').text
        except:
            actual_comment = "Error"
        try:
            commenter_writes = comment.find_element_by_css_selector('a.commenter-publication > span').text
        except:
            commenter_writes = ''
        try:
            writer2.writerow([link, commenter, actual_comment, commenter_writes])
        except:
            writer2.writerow([link.encode('utf-8'), commenter.encode('utf-8'), actual_comment.encode('utf-8'), commenter_writes.encode('utf-8')])
